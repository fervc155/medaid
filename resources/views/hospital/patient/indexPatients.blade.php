            ...
            <tbody>
              ...
              <tr>
                ...
                <td><a href="{{url('/patient/'.$p->dni)}}"  class="btn btn-primary btn-round btn-just-icon btn-sm"><i class="fal fa-user-injured"></i></a>
                  ...
                  <button class="btn btn-danger btn-round btn-just-icon btn-sm btn-confirm-delete" id="paciente-{{$p->dni}}"> <i class="fas fa-times"></i></button>

                  {!! Form::open(['action' => ['PatientController@destroy', $p->dni], 'method' => 'POST']) !!}
                  {{ Form::hidden('_method', 'DELETE') }}
                  {{ Form::submit('Eliminar', ['class' => 'btn-delete  d-none','id'=>'paciente-'.$p->dni]) }}
                  {!! Form::close() !!}
